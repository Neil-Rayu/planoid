<script>
	...
	$: todos = [];
	let todoText;
	$: todoId = totalTodos ? Math.max(...todos.map((t) => t.id)) + 1 : 1;
	...
	$: todoImportantFlag = false;
	$: todoUrgentFlag = false;

	function addTodo() {
		todos = [
			...todos,
			{ id: todoId, name: todoText, importantFlag: todoImportantFlag, urgentFlag: todoUrgentFlag }
		];
		$todoData = [...$todoData, todos.at(-1)];
		todoText = '';
	}
...
<main>
	<div class="task-content">
		...
		<div id="task-wrapper">
			{#each todos as todo}
				<Todo
					bind:todoText={todo.name}
					bind:todoId={todo.id}
					bind:todoImportantFlag={todo.importantFlag}
					bind:todoUrgentFlag={todo.urgentFlag}
				/>
			{/each}
		</div>
